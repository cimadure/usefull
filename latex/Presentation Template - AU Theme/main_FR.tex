...
% Pour projeter en double-écran, j'utilise Dual Screen PDF Viewer :
% https://dspdfviewer.danny-edel.de/
% C'est un logiciel libre, qui marche parfaitement sur tous les systèmes d'exploitation
...
%%% NOTE %%%
% En cas de problèmes avec les diacritiques dans les signets PDF,
% tentez d'utiliser "pdfencoding=utf8" au lieu de "pdfencoding=auto" lors du
% chargement du package hyperref, dans le fichier au/sty/beamerthemeAU.sty
...
\titlegraphic{\includegraphics[width=2cm,]{images/lia_logo.pdf}}
...
\begin{frame}{Plan de la présentation}
    \tableofcontents
%        [hideallsubsections] % ligne à dé-commenter pour cacher les sous-sections dans la table des matières
\end{frame}
...
\begin{frame}
    ...
    % taille arbitraire : \fontsize{<taille>}{<taille>}\selectfont <Mon texte>
    % texte plus petit : \smaller[x] où x est un entier
    % pour compenser interligne dans liste: \setlength\itemsep{0.05cm}
    % \begin{enumerate}
    ...
    \begin{columns}[T,totalwidth=\textwidth] % 
        ...
        \begin{column}{0.33\textwidth}
            ...
            \begin{figure}[H]
                ...
                \href{http://univ-avignon.fr}{\XeTeXLinkBox{\includegraphics[scale=0.2]{au/images/au_col_logo.pdf}}}
                \vspace{-0.5cm}
...
\begin{frame}
    ...
    %     \node[xshift=0cm,yshift=-1.5cm] at (current page.center) {\resizebox{0.8\linewidth}{!}{\input{images/mon_diagramme.tex}}};
    % \end{tikzpicture}
    % voir aussi : https://tex.stackexchange.com/a/89592/31360
    
    % figure couvrant le slide en entier
    % {\definecolor{slidebg}{RGB}{43,43,43}           % définit la couleur de fond
    % \setbeamercolor{background canvas}{bg=slidebg}  % change la couleur de fond
    % \begin{frame}[plain]                            % crée un slide sans décoration
    %     \begin{tikzpicture}[remember picture,overlay]
    %         \node[at=(current page.center)] {
    %             \includegraphics[keepaspectratio, width=\paperwidth, height=\paperheight]
    %                 {images/mon_image.jpg}          % insère l'image, dimensions maximales
    %         };
    %     \end{tikzpicture}
    % \end{frame}}
\end{frame}
...
\begin{frame}
    ...
    % noeud:    \visible<3->{\Vertex[x=-0.00, y= 0.00, label=$v_1$, color=fgVeryLightRed]{v1}}
    % lien:     \only<2>{\Edge[color=black](v1)(v4)}
\end{frame}
...
\begin{frame}
    ...
    % item<1-|handout:1->
    % la valeur zéro permet de masquer complètement l'élément
    % item<1-|handout:0>
\end{frame}
...
\begin{frame}
    ...
    \medskip
    \visible<3>{Il faut remarquer que, quand le texte ci-dessus est voilé, il apparaît tout de même, mais en filigrane. Ceci peut être pratique, par exemple, si le présentateur ne se souvient pas de ce qui vient après \texttt{\textbackslash{}pause}.}
\end{frame}
